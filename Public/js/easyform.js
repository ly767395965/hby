...
(function ($, window, document, undefined)
{
    ...
    _easyinput.prototype = {
        ...
        judge: {
            ...
            "char-chinese": function (ei, v, p)//中文验证
            {
                if (false == /^[\u4E00-\u9FA5]{2,5}$/.test(v))
                    return ei._error("char-chinese");
                else
                    return ei._success_rule("char-chinese");
            },
            "char-mobile": function (ei, v, p)//手机验证
            {
                if (false == /^(1)(3[0-9]|4[0-9]|5[0-9]|7[0-9]|8[0-9])[0-9]{8}$/.test(v))
                    return ei._error("char-mobile");
                else
                    return ei._success_rule("char-mobile");
            },
            "char-idCard": function (ei, v, p)//身份号验证
            {
                if (false == /(^\d{15}$)|(^\d{18}$)|(^\d{17}(\d|X|x)$)/.test(v))
                    return ei._error("char-idCard");
                else
                    return ei._success_rule("char-idCard");
            },
